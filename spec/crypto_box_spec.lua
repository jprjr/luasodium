  ...
  for _,f in ipairs({
    ...
    describe('function ' .. crypto_box_detached, function()
      it('should throw errors on invalid calls', function()
        ...
        assert(pcall(lib[crypto_box_detached],'','','','','') == false)
        assert(pcall(lib[crypto_box_detached],'',string.rep('\0',lib[MACBYTES]),'','','') == false)
        assert(pcall(lib[crypto_box_detached],'',string.rep('\0',lib[MACBYTES]),string.rep('\0',lib[NONCEBYTES]),'','') == false)
        assert(pcall(lib[crypto_box_detached],'',string.rep('\0',lib[MACBYTES]),string.rep('\0',lib[NONCEBYTES]),string.rep('\0',lib[PUBLICKEYBYTES]),'') == false)
      end)
