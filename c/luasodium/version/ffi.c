...
luaopen_luasodium_version_ffi(lua_State*L) {
    if(luaL_loadbuffer(L,ls_version_ffi_implementation,ls_version_ffi_implementation_length,"version.lua") ) {
        return lua_error(L);
    }
    ...
    if(luaL_loadbuffer(L,ls_version_ffi_signatures,ls_version_ffi_signatures_length,"luasodium/version/signatures.lua")) {
        return lua_error(L);
    }
    if(lua_pcall(L,0,1,0)) {
        return lua_error(L);
