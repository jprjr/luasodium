...
int luaopen_luasodium_crypto_sign_ffi(lua_State *L) {
    if(luaL_loadbuffer(L,ls_crypto_sign_ffi_implementation,ls_crypto_sign_ffi_implementation_length,"crypto_sign.lua") ) {
        return lua_error(L);
    }
    ...
    if(luaL_loadbuffer(L,ls_crypto_sign_ffi_signatures,ls_crypto_sign_ffi_signatures_length,"luasodium/crypto_sign/signatures.lua")) {
        return lua_error(L);
    }
    if(lua_pcall(L,0,1,0)) {
        return lua_error(L);
    ...
    lua_newtable(L);
    luasodium_push_functions(L,ls_crypto_sign_functions,lua_gettop(L));
