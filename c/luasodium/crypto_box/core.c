...
ls_crypto_box_keypair(lua_State *L) {
    ...
    if(crypto_box_keypair(pk,sk) == -1) {
        return luaL_error(L,"crypto_box_keypair error");
    }
    /* LCOV_EXCL_STOP */
...
static int
ls_crypto_box_curve25519xsalsa20poly1305_keypair(lua_State *L) {
    ...
    /* LCOV_EXCL_START */
    if(crypto_box_curve25519xsalsa20poly1305_keypair(pk,sk) == -1) {
        return luaL_error(L,"crypto_box_curve25519xsalsa20poly1305_keypair error");
    }
    /* LCOV_EXCL_STOP */

    lua_pushlstring(L,(const char *)pk,crypto_box_curve25519xsalsa20poly1305_PUBLICKEYBYTES);
    lua_pushlstring(L,(const char *)sk,crypto_box_curve25519xsalsa20poly1305_SECRETKEYBYTES);

    sodium_memzero(pk,crypto_box_curve25519xsalsa20poly1305_PUBLICKEYBYTES);
    sodium_memzero(sk,crypto_box_curve25519xsalsa20poly1305_SECRETKEYBYTES);
...
static int
ls_crypto_box_curve25519xsalsa20poly1305(lua_State *L) {
    unsigned char *c      = NULL;
    const unsigned char *m = NULL;
    const unsigned char *nonce = NULL;
    const unsigned char *pk    = NULL;
    ...
    if(noncelen != crypto_box_curve25519xsalsa20poly1305_NONCEBYTES) {
        return luaL_error(L,"wrong nonce size, expected: %d",crypto_box_curve25519xsalsa20poly1305_NONCEBYTES);
    }

    if(pklen != crypto_box_curve25519xsalsa20poly1305_PUBLICKEYBYTES) {
        return luaL_error(L,"wrong public key size, expected: %d",crypto_box_curve25519xsalsa20poly1305_PUBLICKEYBYTES);
    }

    if(sklen != crypto_box_curve25519xsalsa20poly1305_SECRETKEYBYTES) {
        return luaL_error(L,"wrong secret key size, expected: %d",crypto_box_curve25519xsalsa20poly1305_SECRETKEYBYTES);
    }

    clen = mlen + crypto_box_curve25519xsalsa20poly1305_MACBYTES;

    tmp_m = (unsigned char *)lua_newuserdata(L,mlen + crypto_box_curve25519xsalsa20poly1305_ZEROBYTES);
    ...
    c = (unsigned char *)lua_newuserdata(L,clen + crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES);
    ...
    sodium_memzero(tmp_m,crypto_box_curve25519xsalsa20poly1305_ZEROBYTES);
    sodium_memzero(c,crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES);

    memcpy(&tmp_m[crypto_box_curve25519xsalsa20poly1305_ZEROBYTES],m,mlen);

    /* LCOV_EXCL_START */
    if(crypto_box_curve25519xsalsa20poly1305(c,tmp_m,mlen+crypto_box_curve25519xsalsa20poly1305_ZEROBYTES,nonce,pk,sk) == -1) {
        return luaL_error(L,"crypto_box_curve25519xsalsa20poly1305 error");
    }
    /* LCOV_EXCL_STOP */

    lua_pushlstring(L,(const char *)&c[crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES],clen);
    sodium_memzero(tmp_m,mlen + crypto_box_curve25519xsalsa20poly1305_ZEROBYTES);
    sodium_memzero(c,clen + crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES);
    return 1;
}
...
static int
ls_crypto_box_curve25519xsalsa20poly1305_open(lua_State *L) {
    unsigned char *m      = NULL;
    const unsigned char *c = NULL;
    const unsigned char *nonce = NULL;
    const unsigned char *pk    = NULL;
    ...
    if(clen < crypto_box_curve25519xsalsa20poly1305_MACBYTES) {
        return luaL_error(L,"wrong mac size, expected at least: %d",crypto_box_curve25519xsalsa20poly1305_MACBYTES);
    }
    ...
    mlen = clen - crypto_box_curve25519xsalsa20poly1305_MACBYTES;

    tmp_c = (unsigned char *)lua_newuserdata(L,clen + crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES);
    ...
    m = (unsigned char *)lua_newuserdata(L,mlen + crypto_box_curve25519xsalsa20poly1305_ZEROBYTES);
    ...
    sodium_memzero(tmp_c,crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES);
    sodium_memzero(m,crypto_box_curve25519xsalsa20poly1305_ZEROBYTES);

    memcpy(&tmp_c[crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES],c,clen);

    if(crypto_box_curve25519xsalsa20poly1305_open(m,tmp_c,clen+crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES,nonce,pk,sk) == -1) {
        return luaL_error(L,"crypto_box_curve25519xsalsa20poly1305_open error");
    }

    lua_pushlstring(L,(const char *)&m[crypto_box_curve25519xsalsa20poly1305_ZEROBYTES],mlen);
    sodium_memzero(tmp_c,clen + crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES);
    sodium_memzero(m,mlen + crypto_box_curve25519xsalsa20poly1305_ZEROBYTES);
    return 1;
}
...
static int
ls_crypto_box_curve25519xsalsa20poly1305_beforenm(lua_State *L) {
    ...
    /* LCOV_EXCL_START */
    if(crypto_box_curve25519xsalsa20poly1305_beforenm(k,pk,sk) == -1) {
        return luaL_error(L,"crypto_box_curve25519xsalsa20poly1305_beforenm error");
    }
    /* LCOV_EXCL_STOP */

    lua_pushlstring(L,(const char *)k,crypto_box_curve25519xsalsa20poly1305_BEFORENMBYTES);
    sodium_memzero(k,crypto_box_curve25519xsalsa20poly1305_BEFORENMBYTES);
    return 1;
}
...
static int
ls_crypto_box_curve25519xsalsa20poly1305_afternm(lua_State *L) {
    unsigned char *c      = NULL;
    const unsigned char *m = NULL;
    const unsigned char *nonce = NULL;
    const unsigned char *k    = NULL;
    ...
    if(klen != crypto_box_curve25519xsalsa20poly1305_BEFORENMBYTES) {
        return luaL_error(L,"wrong key size, expected: %d",crypto_box_curve25519xsalsa20poly1305_BEFORENMBYTES);
    }
    ...
    /* LCOV_EXCL_START */
    if(crypto_box_curve25519xsalsa20poly1305_afternm(c,tmp_m,mlen+crypto_box_curve25519xsalsa20poly1305_ZEROBYTES,nonce,k) == -1) {
        return luaL_error(L,"crypto_box_curve25519xsalsa20poly1305_afternm error");
    }
    /* LCOV_EXCL_STOP */
...
static int
ls_crypto_box_curve25519xsalsa20poly1305_open_afternm(lua_State *L) {
    unsigned char *m      = NULL;
    const unsigned char *c = NULL;
    const unsigned char *nonce = NULL;
    const unsigned char *k    = NULL;
    ...
    if(klen != crypto_box_curve25519xsalsa20poly1305_BEFORENMBYTES) {
        return luaL_error(L,"wrong public key size, expected: %d",crypto_box_curve25519xsalsa20poly1305_BEFORENMBYTES);
    }
    ...
    if(crypto_box_curve25519xsalsa20poly1305_open_afternm(m,tmp_c,clen+crypto_box_curve25519xsalsa20poly1305_BOXZEROBYTES,nonce,k) == -1) {
        return luaL_error(L,"crypto_box_curve25519xsalsa20poly1305_open_afternm error");
    }
...
static int
ls_crypto_box_curve25519xsalsa20poly1305_seed_keypair(lua_State *L) {
    unsigned char pk[crypto_box_curve25519xsalsa20poly1305_PUBLICKEYBYTES];
    unsigned char sk[crypto_box_curve25519xsalsa20poly1305_SECRETKEYBYTES];
    const unsigned char *seed = NULL;
    size_t seed_len = 0;
    ...
    if(seed_len != crypto_box_curve25519xsalsa20poly1305_SEEDBYTES) {
        return luaL_error(L,"wrong seed length, expected: %d",
          crypto_box_curve25519xsalsa20poly1305_SEEDBYTES);
    }

    /* LCOV_EXCL_START */
    if(crypto_box_curve25519xsalsa20poly1305_seed_keypair(pk,sk,seed) == -1) {
        return luaL_error(L,"crypto_box_curve25519xsalsa20poly1305_seed_keypair error");
    }
    /* LCOV_EXCL_STOP */
...
static const struct luaL_Reg ls_crypto_box_nacl_functions[] = {
    ...
    LS_LUA_FUNC(crypto_box_curve25519xsalsa20poly1305_keypair),
    LS_LUA_FUNC(crypto_box_curve25519xsalsa20poly1305),
    LS_LUA_FUNC(crypto_box_curve25519xsalsa20poly1305_open),
    LS_LUA_FUNC(crypto_box_curve25519xsalsa20poly1305_beforenm),
    LS_LUA_FUNC(crypto_box_curve25519xsalsa20poly1305_afternm),
    LS_LUA_FUNC(crypto_box_curve25519xsalsa20poly1305_open_afternm),
    { NULL, NULL },
};

static const struct luaL_Reg ls_crypto_box_sodium_functions[] = {
    LS_LUA_FUNC(crypto_box_seed_keypair),
    LS_LUA_FUNC(crypto_box_curve25519xsalsa20poly1305_seed_keypair),
...
int luaopen_luasodium_crypto_box_core(lua_State *L) {
    ...
    ls_lua_set_functions(L,ls_crypto_box_nacl_functions,0);
    ls_lua_set_functions(L,ls_crypto_box_sodium_functions,0);
