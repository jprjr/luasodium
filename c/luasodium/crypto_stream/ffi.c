...
int luaopen_luasodium_crypto_stream_ffi(lua_State *L) {
    if(luaL_loadbuffer(L,ls_crypto_stream_ffi_implementation,ls_crypto_stream_ffi_implementation_length,"crypto_stream.lua") ) {
        return lua_error(L);
    }
    ...
    if(luaL_loadbuffer(L,ls_crypto_stream_ffi_signatures,ls_crypto_stream_ffi_signatures_length,"luasodium/crypto_stream/signatures.lua")) {
        return lua_error(L);
    }
    if(lua_pcall(L,0,1,0)) {
        return lua_error(L);
